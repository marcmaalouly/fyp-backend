...
class RenderFromDatabaseData
{
    ...
    {
        $this->mb->addTitle($data['id'], $data['name'], $data['unique_id']);
    }

    private function addLink($data)
    {
        if ($data['parent_id'] === null) {
            $this->mb->addLink($data['id'], $data['name'], $data['href'], $data['unique_id'], $data['icon']);
        }
    }
        ...
        for ($i = 0; $i < count($this->data); $i++) {
            if ($this->data[$i]['parent_id'] == $id) {
                if ($this->data[$i]['slug'] === 'dropdown') {
                    ...
                    $this->mb->addLink($this->data[$i]['id'], $this->data[$i]['name'], $this->data[$i]['href'], $this->data[$i]['unique_id']);
                } else {
    ...
    {
        $this->mb->beginDropdown($data['id'], $data['href'], $data['unique_id'], $data['name'], $data['icon']);
